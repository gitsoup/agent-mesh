...
def validate_state_tree(repo_root: Path, coordination_root: Path | None = None) -> list[str]:
    ...
    errors: list[str] = []
    root_project_file = repo_root / PROJECT_FILE
    coordination_project_file = coordination_root / PROJECT_FILE
    project_file = root_project_file if root_project_file.exists() else coordination_project_file
    contract_root = repo_root if root_project_file.exists() else coordination_root
    ...
    errors.extend(validate_directory(contract_root / ".agentic/work", WorkItem))
    if coordination_root != contract_root:
    ...
    errors.extend(validate_required_paths(repo_root, coordination_root, contract_root))
    ...
    if not errors:
        errors.extend(validate_relationships(repo_root, coordination_root))
    return errors
...
def validate_required_paths(
    ...
    contract_required = [
        contract_root / ".agentic/context/CONTEXT.md",
        contract_root / ".agentic/context/CONTEXT-MAP.md",
        contract_root / ".agentic/workflows",
        contract_root / ".agentic/skills",
    ]
    ...
    if not agents_path.exists():
        ...
        errors.append(
            "Root AGENTS.md is missing Agent Mesh startup routing; "
            "merge .agentic/AGENTS-BOOTSTRAP.md into AGENTS.md"
        )
    for path in contract_required:
        if not path.exists():
            base = contract_root if contract_root != repo_root else repo_root
            try:
...
def validate_relationships(repo_root: Path, coordination_root: Path) -> list[str]:
    errors: list[str] = []
    ...
    for review in reviews:
        ...
        if not (coordination_root / review.context.work_item).exists() and not (repo_root / review.context.work_item).exists():
